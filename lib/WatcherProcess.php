...
use Amp\{ Coroutine, Deferred, Success };
...
class WatcherProcess extends Process {
    ...
    public function __construct(PsrLogger $logger) {
        ...
        $this->logger = $logger;
        $this->procGarbageWatcher = Loop::repeat(100, function() {
    ...
    private function bindCommandServer(string $config) {
        ...
        Loop::onReadable($commandServer, function(...$args) { $this->acceptCommand(...$args); });
        ...
        do {
            ...
            do {
                ...
                switch ($message["action"]) {
                    ...
                    case "stop":
                        (new Coroutine($this->stop()))->when([Loop::class, 'stop']);
                        break;
                }
            } while (1);
        } while (1);
    ...
    private function bindIpcServer() {
        ...
        Loop::onReadable($ipcServer, function(...$args) { $this->accept(...$args); });
